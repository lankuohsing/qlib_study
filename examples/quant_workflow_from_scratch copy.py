...
REBALANCE_DAYS    = 5  # 每 5 个交易日调仓一次，降低日频 Top-K 的过高换手成本
...
def score_equal_weight(df, factors, directions=None):
    factor_values = df[factors]
    if directions is not None:
        factor_values = factor_values.mul(pd.Series(directions), axis=1)
...
def calc_daily_ic_series(df, factor):
    """计算一个因子在给定数据段上的每日截面 Spearman IC。"""
    ic_list = []
    for _, grp in df.groupby(level="datetime"):
        valid = grp[[factor, "LABEL"]].dropna()
        if len(valid) < 10:
            continue
        ic_val, _ = stats.spearmanr(valid[factor], valid["LABEL"])
        if not pd.isna(ic_val):
            ic_list.append(ic_val)
    return pd.Series(ic_list, dtype=float)
...
def run_backtest(test_score, test_ret_wide):
    ...
    """
    # Step 6：每 REBALANCE_DAYS 个交易日取一次合成得分最高的 TOPK 只股票，
    # 非调仓日延续上一期持仓。
    holdings = {}
    prev_holdings = []
    for day_no, (date, grp) in enumerate(test_score.groupby(level="datetime")):
        if day_no % REBALANCE_DAYS == 0 or not prev_holdings:
            prev_holdings = grp.xs(date, level="datetime").nlargest(TOPK).index.tolist()
        holdings[date] = prev_holdings
...
def calc_equal_weight_turnover(prev_set, curr_set):
    """两个等权组合目标权重之间的单边换手率。"""
    if not prev_set:
        return 1.0 if curr_set else 0.0
    if not curr_set:
        return 1.0

    prev_weight = 1.0 / len(prev_set)
    curr_weight = 1.0 / len(curr_set)
    return 0.5 * sum(
        abs(
            (curr_weight if stock in curr_set else 0.0)
            - (prev_weight if stock in prev_set else 0.0)
        )
        for stock in prev_set | curr_set
    )


def run_membership_equal_weight_benchmark(membership_dict, ret_wide, dates):
    """按每日真实 CSI300 成分等权持有，并按成分目标权重变化扣费。"""
    records = []
    prev_set = set()
    available_stocks = set(ret_wide.columns)

    for date in sorted(pd.DatetimeIndex(dates)):
        curr_set = {
            stock
            for stock, spans in membership_dict.items()
            if stock in available_stocks
            and any(pd.Timestamp(start) <= date <= pd.Timestamp(end) for start, end in spans)
        }

        # 与策略回测保持相同时间口径：T 日收益来自 T-1 日确定的持仓。
        if date in ret_wide.index and prev_set:
            valid_stocks = list(prev_set.intersection(available_stocks))
            day_rets = ret_wide.loc[date, valid_stocks].dropna()
            gross_ret = day_rets.mean() if len(day_rets) > 0 else 0.0
        else:
            gross_ret = 0.0

        turnover = calc_equal_weight_turnover(prev_set, curr_set)
...
if __name__ == "__main__":
    ...
    # Qlib 日历中偶尔存在全市场 OHLCV 均为空的占位日期。若保留这些日期，
    # rolling(20) 会被整段 NaN 窗口截断；删除全空行后，unstack 不再生成该日期。
    rows_before_drop = len(raw_df)
    raw_df = raw_df.dropna(how="all")
    print(f"删除 OHLCV 全空占位行: {rows_before_drop - len(raw_df)}")

    # Save the exact OHLCV input used by all downstream factor calculations.
    # CSV is intentionally used here so the data can be reviewed without
    # requiring optional parquet dependencies.
    raw_df_output = os.path.join("outputs", "raw_df.csv")
    raw_df.to_csv(raw_df_output, index=True, date_format="%Y-%m-%d")
    print(f"Raw OHLCV data saved to {os.path.abspath(raw_df_output)}")
    ...
    for factor in FACTOR_COLS:
        s = calc_daily_ic_series(train_df, factor)
        valid_s = calc_daily_ic_series(valid_df, factor)
        ic_records[factor] = {
            "IC均值":   round(s.mean(), 4),
            "IC标准差": round(s.std(),  4),
            ...
            "计算日数": len(s),
            "验证IC均值": round(valid_s.mean(), 4),
        }

    ic_table = pd.DataFrame(ic_records).T
    ic_table["方向"] = np.sign(ic_table["IC均值"]).replace(0, 1).astype(int)
    ic_table["方向稳定"] = (
        np.sign(ic_table["IC均值"]) == np.sign(ic_table["验证IC均值"])
    )
    print(f"IC 分析结果（基于 train 段 {n_train_days} 个交易日）:")
    print(ic_table.to_string())

    valid_factors = ic_table[
        (ic_table["IC均值"].abs() > 0.02) & ic_table["方向稳定"]
    ].index.tolist()
    if valid_factors:
        print(f"\n筛选出有效且训练/验证方向一致的因子: {valid_factors}")
    else:
        print("\n没有因子同时满足 |训练IC| > 0.02 且验证方向一致，退回仅按训练IC筛选")
        valid_factors = ic_table[ic_table["IC均值"].abs() > 0.02].index.tolist()
        if not valid_factors:
            valid_factors = FACTOR_COLS

    factor_directions = ic_table.loc[valid_factors, "方向"].to_dict()
    print(f"因子方向（负 IC 因子乘 -1）: {factor_directions}")
    ...
    # 用训练集 IC 符号统一方向，并要求该方向在验证集上保持一致。
    # 方向只使用训练/验证期确定，不读取测试期信息。
    ...
    print(f"  训练集 R2={lr.score(X_tr, y_tr):.5f}")
    print("  （量化信号 R2 通常 < 0.01，说明股票涨跌难以预测，属正常现象）")
    ...
    print(f"回测逻辑：每 {REBALANCE_DAYS} 个交易日用 T 日因子选 Top-K → T+1 等权持有 → 扣手续费")
    ...
    benchmark_df = run_membership_equal_weight_benchmark(
        membership_dict,
        test_ret_wide,
        ret_results["等权EW"].index,
    )
    benchmark_ret = benchmark_df["net_ret"]
    print(
        f"真实成分等权基准: 平均成分数={benchmark_df['member_count'].mean():.1f}, "
        f"平均换手率={benchmark_df['turnover'].mean():.2%}, 已扣交易费"
    )
    ...
    actual_test_start = benchmark_ret.index.min().date()
    actual_test_end = benchmark_ret.index.max().date()
    print(f"\n  绩效对比（实际可评估期间 {actual_test_start} ~ {actual_test_end}）")
    print(f"{'─'*60}")
    print(f"  {'指标':<10}  {'等权EW':>14}  {'LR':>14}  {'基准（成分等权）':>14}")
    print(f"{'─'*60}")
    for k in perf["等权EW"]:
