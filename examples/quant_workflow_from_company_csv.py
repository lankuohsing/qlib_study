...
从原始 OHLCV 出发，手工走完完整量化链路：
  ...
  Step 1  数据加载        从公司行情数据库导出的 CSV 读取 OHLCV
...
# 全局配置
# ─────────────────────────────────────────────────────────
CSV_PATH  = r"D:\projects\github\qlib_study\outputs\raw_df_company_20200101_20260601.csv"
OUTPUT_DIR = r"outputs\company_csv_workflow"
...
# ─────────────────────────────────────────────────────────
# 将执行代码放在 main 保护内，便于后续安全导入本模块中的辅助函数。
# ─────────────────────────────────────────────────────────
if __name__ == "__main__":
    ...
    if list(raw_df.columns) != expected_columns:
        raise ValueError(f"CSV 列不符合预期: {list(raw_df.columns)}")
    raw_df = raw_df.set_index(["datetime", "instrument"]).sort_index()
    raw_df = raw_df.loc[DATA_START:TEST_END]
    print(f"公司行情 CSV 已加载: {os.path.abspath(CSV_PATH)}")
    ...
    # CSV 已按每只股票的上市日至退市日前一日过滤，这里直接使用全部沪深 A 股。
    ...
    nav_path = os.path.join(OUTPUT_DIR, "nav_curve.csv")
    nav_df.to_csv(nav_path)
    print(f"\n净值曲线已保存 → {nav_path}  shape={nav_df.shape}")

    ic_path = os.path.join(OUTPUT_DIR, "ic_analysis.csv")
    ic_table.to_csv(ic_path)
    print(f"IC 分析已保存 → {ic_path}")
