...
def score_equal_weight(splits, factor_columns):
    return {split: df[factor_columns].mean(axis=1) for split, df in splits.items()}
...
def run_equal_weight_combination(args: argparse.Namespace) -> tuple[dict, object, dict]:
    ...
    scores = score_equal_weight(splits, factor_columns)
