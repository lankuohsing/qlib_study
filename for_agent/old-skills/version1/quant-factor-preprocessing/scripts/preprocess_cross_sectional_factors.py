...
DEFAULT_TRAIN_END = "2017-12-31"
DEFAULT_VALID_START = "2018-01-01"
DEFAULT_VALID_END = "2018-12-31"
DEFAULT_TEST_START = "2019-01-01"
DEFAULT_TEST_END = "2020-08-01"
...
def preprocess_factors(
    ...
    after_dropna = processed.dropna()
    dropped_by_nan = len(processed) - len(after_dropna)
    if after_dropna.empty:
        raise ValueError("因子表在 dropna 后没有剩余样本，请检查因子列和 LABEL 的缺失情况。")

    clean_df, dropped_by_membership = filter_by_membership(after_dropna, membership_df)
    if clean_df.empty:
    ...
    splits = {
        name: segment(clean_df, str(start.date()), str(end.date()))
    ...
    diagnostics = {
        "说明": "因子预处理诊断。处理顺序为每日截面 MAD 去极值、Z-Score、dropna、成员资格过滤、时间切分。",
        "factor_columns": factor_columns,
        "label_column": LABEL_COLUMN,
        "input_shape": list(factor_df.shape),
        "input_nan_rows": nan_rows_before,
        "after_dropna_shape": list(after_dropna.shape),
        "dropped_by_nan": int(dropped_by_nan),
        "dropped_by_membership": int(dropped_by_membership),
        "clean_shape": list(clean_df.shape),
        "total_dropped": int(len(factor_df) - len(clean_df)),
        "clean_date_min": str(clean_df.index.get_level_values("datetime").min().date()),
        "clean_date_max": str(clean_df.index.get_level_values("datetime").max().date()),
        "clean_n_days": int(clean_df.index.get_level_values("datetime").nunique()),
        "clean_n_instruments": int(clean_df.index.get_level_values("instrument").nunique()),
    ...
    return clean_df, splits, diagnostics
...
def save_outputs(
    ...
    clean_csv_gz = out_dir / f"{output_prefix}_clean.csv.gz"
    ...
    clean_df.to_csv(clean_csv_gz, compression="gzip")
    splits["train"].to_csv(train_csv_gz, compression="gzip")
    splits["valid"].to_csv(valid_csv_gz, compression="gzip")
    splits["test"].to_csv(test_csv_gz, compression="gzip")
    clean_df.head(preview_rows).to_csv(preview_csv)
...
def print_preprocessing_summary(diagnostics: dict[str, Any]) -> None:
    ...
    clean_shape = tuple(diagnostics["clean_shape"])
    print(
        f"处理前  shape = {input_shape}  "
        f"NaN 行数 = {diagnostics['input_nan_rows']}"
    )
    print(
        f"处理后  shape = {clean_shape}   "
        f"丢弃了 {diagnostics['total_dropped']} 行"
    )
    print(f"  首个有效日期: {diagnostics['clean_date_min']}")
    print(f"  末个有效日期: {diagnostics['clean_date_max']}")
...
def main() -> None:
    ...
    try:
        ...
        clean_df, splits, diagnostics = preprocess_factors(
            factor_df=factor_df,
